...
class MeaageListActivity : BaseStyleActivity() {

    val instance: Model by lazy { Model.getInstance() }
    val useSP: UserSP by lazy { instance.userSP }

    val adapter: MyMessageAdapter by lazy { MyMessageAdapter(this@MeaageListActivity) }
            ...
            override fun onNext(t: ResponseBody?) {
                ...
                if (json.status == 200) {
                    ...
                    if (json.errorMsg.equals("用户未登录")) {
                        startActivity(Intent(this@MeaageListActivity, LoginActivity::class.java))
                    } else {
                        ToastUtil.showShort(this@MeaageListActivity, json.errorMsg)
                    }
                }
            }
